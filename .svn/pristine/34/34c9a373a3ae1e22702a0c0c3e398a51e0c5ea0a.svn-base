...
<![endif]-->
	...
	function addUerInfo(){
		...
		$.ajax({
			type: "POST",
			url: "../user/adduserbychannel",
			data: {username: username, password: password, cnname: cnname, usertype: usertype,chid: chid},
			dataType: "text",
	...
	function dataList(starttime,endtime){
		...
		pageBar = $("#pageBar").myPagination({
			...
			ajax : {
				on : true,
 				type: "POST",  
				url: "../user/channeltouser",
				param:"pageSize=20&chid="+chid,
				dataType: "json",
		...
		if (data && data.list) {
			...
			$('#dataList').append(
				...
				/* 	'<td align="left"><a href="channel_info.jsp?userid='+row.id+'">查看渠道</a></td>' + */
					'<td align="left">'+'<a onClick="show(\''+row.username+'\',\''+row.password+'\','+row.usertype+',\''+row.cnname+'\',\''+row.id+'\');" data-toggle="modal">修改</a>|<a onClick="deleteuserinfo(\''+row.id+'\');">删除</a></td>' +
					'</tr>');
			});
		}else{
