	...
	<div class="container-fluid">
		<div class="page-content page-content-popup">
			...
			<div class="page-fixed-main-content">
					...
					<div class="portlet-body">
						<form action="#" class="form-horizontal form-bordered">
							<div class="form-group">
								...
								<label class="control-label col-md-1">账号类型：</label>
								<div class="col-md-2">
									<input id ="querytype" type="text" class="form-control">
								</div>
								<div class="col-md-2">
									<div class="col-md-12">
				...
				<div id="responsive" class="modal fade" tabindex="-1" data-width="760">
					<div class="modal-body">
						<div class="row">
							<div class="col-md-12">
								<h4>添加账号</h4>
								<p>用户名： <input class="form-control" type="text" id="username"> </p>
								<p>密码： <input class="form-control" type="text" id="password"> </p>
								<p>中文名： <input class="form-control" type="text" id="cnname"> </p>
								<p>账号类型： <input class="form-control" type="text" id="usertype"> </p>
							</div>
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn green" onclick="addUerInfo();">保存</button>
				...
				<div id="chge_responsive" class="modal fade" tabindex="-1" data-width="760">
					<div class="modal-body">
						<div class="row">
							<div class="col-md-12">
								<h4>修改账号</h4>
								<p>用户名： <input class="form-control" type="text" id="chgeusername"> </p>
								<p>密码： <input class="form-control" type="text" id="chgepassword"> </p>
								<p>中文名： <input class="form-control" type="text" id="chgecnname"> </p>
								<p>账号类型： <input class="form-control" type="text" id="chgeusertype"> </p>
							</div>
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn green" onclick="updateUerInfo();">保存</button>
						<button type="button" data-dismiss="modal" class="btn btn-outline dark">取消</button>
					</div>
				</div>
				
				<div id="chgechannel_responsive" class="modal fade"  data-width="760">
					<div class="modal-body">
						<div class="row">
							<div class="col-md-12">
								<h4>选择渠道</h4>
								<select id="select2-single-input-sm" class="">
 								</select>
							</div>
						</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn green" onclick="AddChannelByUserid();">保存</button>
						<button type="button" data-dismiss="modal" class="btn btn-outline dark">取消</button>
					</div>
				</div>
...
<![endif]-->
	...
	function addUerInfo(){
		...
		$.ajax({
			...
			data: {username: username, password: password, cnname: cnname, usertype: usertype},
			dataType: "text",
	...
	function choiceChannel(idi) {
		var select = null; // '<option value="CT">Connecticut</option>'+
		id =idi;
      $.ajax({
 			type: "POST",
 		...
 		//	param:"id="+id,
 			dataType: "json",
 			success:channelListAjaxCallBack,
 			
 		});
         
		
	}
		function channelListAjaxCallBack(data){
					select = null;
					$("#select2-single-input-sm").empty();
				if (data && data.list){
				$.each(data.list, function(i, row){
		 				select += '<option value="'+row.channelid+'">'+row.channelid+'</option>';
		 			});
				}
				$("#select2-single-input-sm").append(select).select2();
		}
	...
	if (data && data.list) {
			...
			$('#dataList').append(
					...
					'<td align="left">' + row.usertype + '</td>' +
					'<td align="left">' + row.cnname + '</td>' +
					'<td align="left"><a href="channel_info.jsp?userid='+row.id+'">查看渠道</a></td>' + 
					'<td align="left">'+'<a onClick="show(\''+row.username+'\',\''+row.password+'\','+row.usertype+',\''+row.cnname+'\',\''+row.id+'\');" data-toggle="modal">修改</a>|<a onClick="deleteuserinfo(\''+row.id+'\');">删除</a>|<a onClick="showchannal(\''+row.id+'\');">选择渠道</a></td>' +
					'</tr>');
			});
		}else{
	...
	*/ 
	...
	function updateUerInfo(){
		var username = $('#chgeusername').val();
		var password = $('#chgepassword').val();
		var cnname = $('#chgecnname').val();
		var usertype = $('#chgeusertype').val();
		id = idi;
		console.info(id);
		$.ajax({
			type: "POST",
			url: "../user/updateuserinfo",
			data: {username: username, password: password, cnname: cnname, usertype: usertype, id: id},
			dataType: "text",
			success: function(data){
				if(data > 0){
					location.reload();
	...
	function AddChannelByUserid(){
		id = idi;
		var channelid = $('#select2-single-input-sm').val();
		console.info(channelid);
		$.ajax({
			type: "POST",
			url: "../user/addchannelbyuserid",
	 ...
	 function show(username,password,usertype,cnname,id){
			console.info("==========================1");
			$('#chgeusername').val(username);
			$('#chgepassword').val(password);
			$('#chgecnname').val(cnname);
			$('#chgeusertype').val(usertype);
			$('#chge_responsive').modal('show');
			idi =id;
			console.info(idi);
		}	
	 function showchannal(id){
			$('#chgechannel_responsive').modal('show');
			idi = id;
			choiceChannel(idi);
		}	
	...
	function dataList(starttime,endtime){
		userName=$('#queryname').val();
		usertype=$('#querytype').val();
...
// 				type: "POST",  
				url: "../user/userlist",
				param:"pageSize=20&userName="+userName+"&usertype="+usertype,
				dataType: "json",
	...
	function appListAjaxCallBack(data){
		...
		if (data && data.list) {
			...
			$('#dataList').append(
					...
					'<td align="left">' + row.usertype + '</td>' +
					'<td align="left">' + row.cnname + '</td>' +
				 	'<td align="left"><a href="channel_info_byuser.jsp?userid='+row.id+'">查看渠道</a></td>' + 
					'<td align="left">'+'<a onClick="show(\''+row.username+'\',\''+row.password+'\','+row.usertype+',\''+row.cnname+'\',\''+row.id+'\');" data-toggle="modal">修改</a>|<a onClick="deleteuserinfo(\''+row.id+'\');">删除</a>|<a onClick="showchannal(\''+row.id+'\');">选择渠道</a></td>' +
					'</tr>');
			});
		}else{
